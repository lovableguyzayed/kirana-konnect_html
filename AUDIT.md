...
| Item | What was done |
|---|---|
| **Pending credits page was static** | Now renders live debtors from `/api/customers/with-balance` with overdue/due/current bucketing, live summary + tab counts, tap-to-call, ledger link, and **Collect** which records a payment and refreshes in place. |
| **Expiry alerts page was static** | Now renders live expired/expiring stock from `/api/expired-products` with live counts and a Remove action for expired items. |
| **Staff was a pure mock** | Real `Staff` model + `GET/POST/DELETE /api/staff`; a My Team card on the staff page lists, adds and removes members; the cart's "Bill Generated By" dropdown now lists the real owner + team. Seeder adds 3 staff. |
| **Cart bill header was hardcoded** | Shows the real shop name/GST from the account. |
...
| P3 | Settings page toggles are visual-only | Notification-settings API exists; the page's switch markup has no stable ids to bind. Nothing is written incorrectly - they simply don't persist. |
| P3 | Staff attendance (clock in/out) is visual-only | Team management is real; attendance tracking would need its own model. |
